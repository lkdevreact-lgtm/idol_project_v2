...
import { MdAdd, MdEdit, MdDelete, MdClose, MdCheck, MdSearch, MdCardGiftcard } from "react-icons/md";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="relative w-full max-w-md mx-4 bg-[#1a1820] border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-6 flex flex-col gap-4">
          {error && <div className="text-red-400 text-xs bg-red-400/10 border border-red-400/20 px-3 py-2 rounded-lg">⚠️ {error}</div>}
          
          <div>
            <label className="text-xs text-white/50 mb-1 block">Gift ID (Số) *</label>
            <input
              type="number"
              value={giftId}
              disabled={!!initial}
          ...
          <div>
            <label className="text-xs text-white/50 mb-1 block">Tên Quà *</label>
            <input
              value={giftName}
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/10 bg-white/5">
          <button onClick={onClose} className="px-4 py-2 rounded-lg border border-white/10 text-white/60 hover:text-white text-sm transition">Hủy</button>
          <button onClick={handleSave} className="px-5 py-2 rounded-lg bg-pink-500 hover:bg-pink-600 text-white text-sm font-semibold transition flex items-center gap-2">
            <MdCheck size={18} />
            Lưu
          </button>
        </div>
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
    <div className="w-full max-w-sm mx-4 bg-[#1a1820] border border-red-500/30 rounded-2xl shadow-2xl p-6 flex flex-col gap-4">
      ...
      <p className="text-white/60 text-sm">
        Bạn có chắc muốn xóa quà <span className="text-white font-semibold">"{name}"</span> không?
      </p>
      <div className="flex gap-3 justify-end">
        <button onClick={onClose} className="px-4 py-2 rounded-lg border border-white/10 text-white/60 hover:text-white text-sm transition">Hủy</button>
        <button onClick={onConfirm} className="px-5 py-2 rounded-lg bg-red-500 hover:bg-red-600 text-white text-sm font-semibold transition">Xóa</button>
      </div>
    </div>
  </div>
);
  ...
  return (
    <div className="w-full h-full overflow-hidden flex flex-col text-white">
      ...
      <div className="shrink-0 flex items-center justify-between px-6 py-5 border-b border-white/10">
        <div>
          <h1 className="text-2xl font-bold text-white">🎁 Quản lý Quà tặng</h1>
          <p className="text-sm text-white/40 mt-0.5">Tổng cộng {gifts.length} loại quà đã thu thập</p>
        </div>
        <button
          onClick={() => setModal({ mode: "add" })}
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-pink-500 text-white font-semibold text-sm shadow-lg shadow-pink-500/25 hover:bg-pink-600 transition"
        >
      ...
      {/* Toolbar */}
      <div className="shrink-0 px-6 py-4 flex items-center gap-4 bg-white/5 border-b border-white/5">
        <div className="relative flex-1 max-w-md">
          <MdSearch size={20} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30" />
          <input
            value={search}
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        {loading ? (
          <div className="h-full flex items-center justify-center"><div className="w-8 h-8 border-2 border-pink-500 border-t-transparent rounded-full animate-spin" /></div>
        ) : filtered.length === 0 ? (
              ...
              return (
                <div 
                  ...
                  className={`group relative flex flex-col p-5 rounded-2xl border transition-all duration-300 ${
                    isActive 
                      ? "bg-white/5 border-white/10 hover:border-pink-500/50 hover:bg-white/8 shadow-lg shadow-black/20" 
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-4">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center border transition-colors ${
                        isActive ? "bg-pink-500/10 border-pink-500/20 text-pink-400" : "bg-white/5 border-white/10 text-white/30"
                      }`}>
                        {gift.image ? (
                      ...
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          <h3 className="text-base font-bold text-white">{gift.giftName}</h3>
                          {isUsed && (
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <p className="text-[11px] text-white/30 font-mono italic">ID: {gift.giftId}</p>
                          {gift.diamonds > 0 && (
                    ...
                    <button 
                      ...
                      className={`relative w-10 h-5 rounded-full transition-colors duration-300 ${isActive ? "bg-pink-500" : "bg-white/10"}`}
                    >
                    ...
                    <div className="flex items-center gap-2">
                       {gift.maxRepeatCount > 1 && (
                         <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-orange-500/10 border border-orange-500/20 text-orange-400">
                            ...
                            <span className="text-[11px] font-bold">Record: {gift.maxRepeatCount}</span>
                         </div>
                       )}
                    </div>
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-200">
                      <button onClick={() => setModal({ mode: "edit", data: gift })} className="p-2 text-white/30 hover:text-cyan-400 hover:bg-cyan-400/10 rounded-lg transition"><MdEdit size={18} /></button>
                      <button onClick={() => setDeleteTarget(gift)} className="p-2 text-white/30 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition"><MdDelete size={18} /></button>
                    </div>
                  </div>
                </div>
              );
      ...
      {modal && (
        <GiftModal
          initial={modal.data}
          onSave={async (data) => {
             const res = modal.mode === 'add' ? await addGift(data) : await updateGift(data.giftId, { giftName: data.giftName });
